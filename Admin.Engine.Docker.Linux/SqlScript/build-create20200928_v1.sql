    ...
    if exists (select 1 from sys.objects where object_id = OBJECT_ID(N'[fk_supplier_id]') and parent_object_id = OBJECT_ID(N'item'))
alter table item  drop constraint fk_supplier_id
...
alter table supplier_contact  drop constraint fk_supplier_id
    ...
    if exists (select * from dbo.sysobjects where id = object_id(N'item') and OBJECTPROPERTY(id, N'IsUserTable') = 1) drop table item
